...
cat jupiter.template.yaml | DOLLAR='$' envsubst > docker-compose.yml
...
docker stop `docker ps -aq -f name=jupiter_*`
docker rm `docker ps -aq -f name=jupiter_*`
docker network prune -f
docker-compose -p "jupiter" up -d
